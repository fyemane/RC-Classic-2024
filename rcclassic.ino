...
const int ESC_PWM_FREQ = 100;     // Recall that Arduino Uno is ~490 Hz. Official ESP32 example uses 5,000Hz
...
double esc_min_perc;
double esc_max_perc;
int esc_min = ESC_MIN_DC_PERC * ESC_MAX_DUTY_CYCLE;
int esc_max = ESC_MAX_DC_PERC * ESC_MAX_DUTY_CYCLE;
...
int driveled = 0b0001;
int steeringled = 0b0010;
int throttle_70_led = 0b1000;
int led = 0;
...
void processGamepad(ControllerPtr ctl) {
    ...
    // DRIVE
    if (!driveLocked) {
      // set mode to 70% throttle

      if (throttle_set) {
        if (throttle_70 && throttle_set) {
          ESC_MIN_DC_PERC = ESC_MIN_DC_PERC_70;
          ESC_MAX_DC_PERC = ESC_MAX_DC_PERC_70;
        } 
        // set mode to 100% throttle
        if (!throttle_70 && throttle_set) {
          ESC_MIN_DC_PERC = ESC_MIN_DC_PERC_100;
          ESC_MAX_DC_PERC = ESC_MAX_DC_PERC_100;
        }
        esc_min = ESC_MIN_DC_PERC * ESC_MAX_DUTY_CYCLE;
        esc_max = ESC_MAX_DC_PERC * ESC_MAX_DUTY_CYCLE;
        throttle_set = 0;
      }

      // increase the speed range by pressing UP on the dpad
      if (upPressed) {
        if (ESC_MIN_DC_PERC >= ESC_MIN_DC_PERC_100 && ESC_MAX_DC_PERC <= ESC_MAX_DC_PERC_100) {
          ESC_MIN_DC_PERC -= 0.005;
          ESC_MAX_DC_PERC += 0.005;
          esc_min = ESC_MIN_DC_PERC * ESC_MAX_DUTY_CYCLE;
          esc_max = ESC_MAX_DC_PERC * ESC_MAX_DUTY_CYCLE;
        }
      }

      // decrease the speed range by pressing down on the dpad
      if (downPressed) {
        if (ESC_MIN_DC_PERC <= 0.5 * (ESC_MIN_DC_PERC_100 + ESC_MAX_DC_PERC_100) && ESC_MAX_DC_PERC >= 0.5 * (ESC_MIN_DC_PERC_100 + ESC_MAX_DC_PERC_100)) {
          ESC_MIN_DC_PERC += 0.0005;
          ESC_MAX_DC_PERC -= 0.0005;
          esc_min = ESC_MIN_DC_PERC * ESC_MAX_DUTY_CYCLE;
          esc_max = ESC_MAX_DC_PERC * ESC_MAX_DUTY_CYCLE;
        }
      }
      ...
      // Map the analog values to the appropriate range (-512 to 512 in this example)
      mappedYValue = map(yAxisValue, -512, 512, esc_min, esc_max);
      mappedYValue = (-1) * mappedYValue + esc_min + esc_max;
    ...
    // if Button L2 is pressed, acceleration goes to 0
    if (buttonL2) {
    ...
    if (!steeringLocked) {
      ...
      // steering range depends on trim
      ...
      // Map the analog values to the appropriate range (-512 to 512 in this example)
      mappedrXValue = map(rxAxisValue, -512, 512, str_min / STR_MAX_DUTY_CYCLE, str_max ? );
    ...
    if (throttle_70)
      led = led | throttle_70_led;
    else
      led = led & ~throttle_70_led;


    Serial.printf("Mapped Steer rX:: %d, Mapped Drive Y: %d, throttle_70: %d, Drive lock: %d, Steer lock: %d, Str cp %.5f, Str c %d, Str minp %.5f, Str min %d, Str max p %.5f, Str max %d, LED %d\n",
                 mappedrXValue, 
                 mappedYValue, 
                 throttle_70, 
                 driveLocked,
    ...
    // send PWM signals to the steering and esc pins 
